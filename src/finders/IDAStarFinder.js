...
IDAStarFinder.prototype.findPath = function(startX, startY, endX, endY, grid) {
  ...
  // Heuristic helper:
  const h = (a, b) => this.heuristic(Math.abs(b.x - a.x), Math.abs(b.y - a.y));

  // Step cost from a to b:
  const cost = (a, b) => (a.x === b.x || a.y === b.y ? 1 : Math.SQRT2);
  ...
  const search = (node, g, cutoff, route, depth) => {
    ...
    const neighbours = grid.getNeighbors(node, this.diagonalMovement); //Disable warning: Expected a conditional expression and instead saw an assignment
